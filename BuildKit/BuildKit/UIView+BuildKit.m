...
@implementation UIView (BuildKit)
...
+ (instancetype)build_viewWithFrame:(CGRect)cgrect andBGColor:(UIColor *)bgColor andCornerRadius:(CGFloat)cornerRadius andSubV:(UIView *)subView{
    UIView *resultView = [[UIView alloc] initWithFrame:cgrect];
    resultView.backgroundColor = bgColor;
    if (cornerRadius > 0) {
        resultView.layer.cornerRadius = cornerRadius;
        resultView.layer.masksToBounds = YES;
    }
    
    if (subView) {
        [subView addSubview:resultView];
    }
    return resultView;
}
